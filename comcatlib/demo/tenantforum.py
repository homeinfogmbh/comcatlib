...
def create_topics(users: Sequence[User], threads: Sequence[dict]) -> None:
    ...
    for topic_index, (op, thread) in enumerate(randzipfill(users, threads)):
        topic = create_topic(
            op, thread["title"], thread["text"], len(threads) - topic_index
        )
...
def delete_topics(customer: Customer) -> None:
    ...
    for topic in (
        Topic.select().join(User).join(Tenement).where(Tenement.customer == customer)
    ):
...
def create_topic(user: User, title: str, text: str, offset: int) -> Topic:
    ...
    timestamp = randdate(datetime.now() - timedelta(days=offset), datetime.now())
...
def create_response(user: User, topic: Topic, text: str, index: int) -> None:
    ...
    timestamp = randdate(
        topic.created, topic.created + timedelta(hours=index + 1, minutes=index + 1)
    )
