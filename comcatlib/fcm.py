...
CAPTIONS = {URLCode.NEWS: "Neue News", URLCode.DOWNLOAD: "Neuer Download"}
...
def expand_groups(groups: Iterable[Union[Group, int]]) -> set[Union[Group, int]]:
...
def groups_users(groups: Iterable[Union[Group, int]]) -> Iterator[Union[User, int]]:
    """Yield users that are members of the respective groups."""

    for member in GroupMemberUser.select().where(GroupMemberUser.group << groups):
        yield member.user
...
def multicast_base_chart(base_chart: BaseChart, url_code: URLCode) -> BatchResponse:
    """Multicast base chart to users."""

    return multicast_message(
        [
            token.token
            for token in get_tokens(set(affected_users_by_base_chart(base_chart)))
        ],
        url_code=url_code,
...
def multicast_message(
    tokens: Iterable[str], *, url_code: URLCode, title: str, body: str
) -> BatchResponse:
    ...
    return send_multicast(
        MulticastMessage(
            ...
            android=AndroidConfig(
                notification=AndroidNotification(click_action="FCM_PLUGIN_ACTIVITY")
            ),
        )
    )
