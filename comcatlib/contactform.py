...
def get_contact_emails(user: User, json: dict) -> Iterator[EMail]:
    ...
    html = tostring(get_html_body(user, json), encoding="unicode", method="html")

    for recipient in get_recipients(user.tenement.customer):
        yield EMail(
            "Neue Kontaktanfrage", SENDER, recipient, reply_to=json["email"], html=html
        )
...
def send_contact_mails(user: User, json: dict) -> None:
    ...
    get_mailer().send(get_contact_emails(user, json))
