...
def notify_user(email: str, passwd: Optional[str] = None) -> None:
    ...
    get_mailer().send([EMail(SUBJECT, SENDER, email, plain=get_body(email, passwd))])


def get_customer_emails(user_registration: UserRegistration) -> Iterator[EMail]:
    """Yields emails to send."""

    html = tostring(to_html(user_registration), encoding="unicode", method="html")

    for notification_email in RegistrationNotificationEmails.select().where(
        RegistrationNotificationEmails.customer == user_registration.customer
    ):
        yield EMail(NOTIFICATION_SUBJECT, SENDER, notification_email.email, html=html)
